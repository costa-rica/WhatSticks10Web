...
from ws_models import sess, text, Users, CommunityPosts
...
def view_post(post_dir_name):
    ...
    post = sess.get(CommunityPosts,post_id)
...
def create_post():
    ...
    if request.method == 'POST':
        ...
        if formDict.get('dropdown_upload_zip_type') == 'origin_from_word':
            ...
            new_blogpost = CommunityPosts(user_id=current_user.id)
        ...
        #     # create new_blogpost to get post_id number
        #     new_blogpost = CommunityPosts(user_id=current_user.id, date_published =date_published_datetime)
        #     sess.add(new_blogpost)
        #     sess.commit()
...
def blog_edit(post_id):
    ...
    post = sess.query(CommunityPosts).filter_by(id = post_id).first()
    title = post.title
